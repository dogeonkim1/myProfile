        ...
        <section className="mt-35 px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-10 px-4">
                {cardData.map((card, index) => (
                    ...
                    viewport={{once:true, amount:0.3}}
                    >
                     <div className={`absolute -top-6 left-6 px-4 py-1 rounded-full text-black font-semibold ${card.labelColor}`}>
                         {card.title}
                     </div>
                        <div className="flex-grow flex items-center justify-center py-6">
                            <Image src={card.image} alt={card.title} width={200} height={200} className="object-contain" />
                        </div>
